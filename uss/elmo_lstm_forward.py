"""
A stacked forward only LSTM with skip connections between layers.

Modified from allennlp/modules/elmo_lstm.py.
"""
...
class ElmoLstmForward(_EncoderBase):
    """
    A stacked, forward only LSTM which uses
    ...
    def __init__(self,
        ...
        super(ElmoLstmForward, self).__init__(stateful=False)              # change 'stateful' flag to be False
                                                                    # so that hidden_state can be externally provided
    ...
    def forward(self,  # pylint: disable=arguments-differ
                ...
                hidden_state: Optional[RnnState] = None) -> torch.Tensor:
        """
        Parameters
        ----------
        ...
        A tuple of
        tensors of shapes (num_layers, batch_size, hidden_size) and
        (num_layers, batch_size, memory_size), representing the final hidden state and memory
        state of an LSTM-like RNN.
        """
        batch_size, total_sequence_length = mask.size()
        stacked_sequence_output, final_states, restoration_indices = \
            self.sort_and_run_forward(self._lstm_forward, inputs, mask, hidden_state)       # add 'hidden_state' here
...
#        self._update_states(final_states, restoration_indices)
        ...
        return stacked_sequence_output.index_select(1, restoration_indices), \
             tuple([state.index_select(1, restoration_indices).detach() for state in final_states])        # detach final states
    ...
    def _lstm_forward(self,
        ...
        if initial_state is None:
            ...
            hidden_states = list(zip(initial_state[0].split(1, 0), initial_state[1].split(1, 0)))
                                   # list of tuples, each one is a (hidden, memory) tuple for that layer
        ...
        for layer_index, state in enumerate(hidden_states):
            ...
            if state is not None:
                forward_hidden_state = state[0]
                forward_memory_state = state[1]
            ...
            final_states.append((forward_state[0],
                                 forward_state[1]))
