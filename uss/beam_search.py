...
class Beam:
    def __init__(self, init_K, vocab, init_ids, device=None, **kwargs):
        assert 1 <= init_K <= len(vocab), 'Initial beam size should be in [1, len(vocab)]!'
    ...
    def retrieve(self, k, seq_len=-1):
        ...
        assert 1 <= k <= self.K[seq_len], 'k must be in [1, the total number of beams at seq_len]!'
    ...
    def lmscore(self, bulist, K, LMModel, word_list=None, subvocab=None, clustermask=None, renorm=False, temperature=1):
        ...
        with torch.no_grad():
            ...
            subprobs, probs, hn = prob_next_1step(LMModel, batch_text, hn=batch_hn,
                                                  subvocab=subvocab, clustermask=clustermask, onscore=False,
                                                  renorm=renorm,
                                                  temperature=temperature)
        # convert the hidden state tuple into a list of tuples, corresponding to each beam sequence
        hn = list(
            zip(torch.chunk(hn[0], chunks=len(onbeam_ids), dim=1), torch.chunk(hn[1], chunks=len(onbeam_ids), dim=1)))
        lm_cum_logprob = subprobs.new_tensor([bulist[i].lm_score for i in onbeam_ids]).unsqueeze(1) + torch.log(
            subprobs)
    ...
    def combscoreK(self, bulist, K, template_vec, ee, LMModel,
        ...
        Input:
            stopbyLMeos: whether to use the LM '<eos>' to solely decide end of sentence, i.e. when '<eos>' gets the
            highest probability from the LM, remove the generated sentence out of beam. Default: False.
        ...
        onbeam_ids = list(range(
            len(bulist)))  # keep track of sequences on beam that have not aligned to the end of the source sequence
        ...
        for (i, bu) in enumerate(bulist):
            try:
                ...
                sim_cum_allbeam = sim_cum_logprob if sim_cum_allbeam is None else torch.cat(
                    [sim_cum_allbeam, sim_cum_logprob])
        ...
        if len(bulist) == 1 and bulist[0].word_id is None:
            ...
            # convert the hidden state tuple into a list of tuples, corresponding to each beam sequence
            hn = list(zip(torch.chunk(hn[0], chunks=len(onbeam_ids), dim=1),
                          torch.chunk(hn[1], chunks=len(onbeam_ids), dim=1)))
            lm_cum_logprob = subprobs.new_tensor([bulist[i].lm_score for i in onbeam_ids]).unsqueeze(1) + torch.log(
                subprobs)
            lm_cum_logprob = lm_cum_logprob.view(-1)  # this is the cumulative log probabilities
